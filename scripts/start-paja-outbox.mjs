...
function parseArgs(argv) {
  const values = { ...DEFAULTS, relayUrls: [...DEFAULTS.relayUrls] };
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (argument === '--relay-url' && value) values.relayUrls.push(value);
    else throw new Error(`Unknown or incomplete argument: ${argument}`);
    index += 1;
  }
  if (!Number.isInteger(values.port) || values.port < 1 || values.port > 65_535) {
    throw new Error('The Paja outbox port must be an integer from 1 to 65535');
  }
  return values;
}
...
async function loadBrowserHost() {
  const pajaEntry = fileURLToPath(import.meta.resolve('@kehto/paja'));
  return readFile(join(dirname(pajaEntry), 'browser-host.js'), 'utf8');
}
...
async function main() {
  ...
  const options = normalizePajaOptions({
    ...
    simulation: {
      ...
      relay: { mode: 'live', urls: args.relayUrls },
      storage: { mode: 'memory' },
    },
  });
  ...
  const browserHost = await loadBrowserHost();
  ...
  console.log(`Nappagochi Outbox Paja: http://${args.host}:${args.port}/`);
  console.log(`Napplet target: ${args.targetUrl}`);
  console.log(`Live relay hints: ${args.relayUrls.join(', ')}`);
  console.log('No local relay-only or local relay mirror config is injected.');
}
